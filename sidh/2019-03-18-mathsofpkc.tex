...
\begin{frame}{VDFs from groups of unknown order}
  \begin{block}{Interactive verification protocol \parencite{Wesolowski}}
    ...
    \item Verifier chooses a prime \emph{$ℓ$} in a set of \emph{small
        primes} $\mathcal{P}$;
    \item Prover computes \emph{$2^T=aℓ+b$}, sends \emph{$g^{2^T},g^a,b$} to
      verifier;
...
\begin{frame}{VDFs from isogenies and pairings}
  \centering
  \begin{tikzpicture}[ampersand replacement=\&]
    \matrix(m)[matrix of math nodes,row sep=3em,column sep=4em]{
      X_1 × Y_2 \& Y_1 × Y_2\\
...
\begin{frame}{Security}
  \begin{description}
  \item[Obvious attack:] Pairing inversion must be hard (non
    post-quantum).

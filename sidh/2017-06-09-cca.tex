...
\title[Isogeny-based cryptography]{Open problems in isogeny-based cryptography}
\author{Luca De Feo}
\date[June 8, 2017 --- CCA]{June 8, 2017, S\'eminaire CCA, Paris}
      ...
      \begin{tikzpicture}[xscale=1.5,yscale=1.5]
        ...
        \begin{uncoverenv}<1,3>
          \begin{scope}[blue,every loop/.style={looseness=50}]
            \path (1) edge (20) edge[bend left=10] (16z) edge[bend right=5] (81z);
            \path (20) edge[loop above] (20) edge[loop below] (20);
            \path (16z) edge[bend left=5] (81z) edge (77z);
            \path (81z) edge (20z);
...
\begin{frame}
  ...
  \begin{block}{Ephemeral key recovery (total break)}
    ...
    \item[Equivalent to] computing the endomorphism rings of both $E_0$
      and $E_0/\cyc{R}$.\footcite{galbraithsecurity}
    \end{description}
  \end{block}
...
\begin{frame}
  ...
  \begin{block}{Other security models}
    ...
    \item[] Attack on partially leaked
      keys.\footcite{gelin2017sidh,ti2017sidh}
    \end{description}
  \end{block}
...
\begin{frame}
  ...
  \textbf{Open problem:} Efficient signatures, \dots
\end{frame}

...
\author{Luca De Feo}
\date[AfricaCrypt, May 25, 2017]{May 25, 2017, AfricaCrypt, Dakar}
...
\frame[plain]{
  \begin{tikzpicture}[remember picture,overlay]
    \node[at=(current page.center)] {
      \includegraphics[height=\paperheight]{david.jpg}
    };
    \draw[->,very thick,red!90!black] (current page.center) ++(-2.2,-3)
    node {\parbox{4cm}{\centering\huge\bf Isogeny\\cycle\\in Brittany}} ++(1.2,0) -- ++(1.8,0);
    \draw[->,thin,red] (current page.center) ++(-1,0.9)
    node {\parbox{2cm}{\centering\scriptsize (post)quantum\\physicist}} ++(-0.1,0.3) -- ++(-1,0.5);
  \end{tikzpicture}
}
...
\begin{frame}
  \frametitle{Structure of the graph\footcite{pizer1,pizer2,kohel,fouquet+morain02}}
  ...
  \begin{block}{The graph of isogenies of \alert{prime} degree \alert{$\ell\ne p$}}
    ...
    \item The graph is an \emph{optimal expander} (random walks \textit{mix} rapidly).
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \emph{(alternative) fact:} Having a \emph{weak DLP} is not isogeny invariant.
...
\begin{frame}
  ...
  \begin{itemize}
  \item Various instances proposed (e.g., matrix groups);
  \item Not \textit{as} broken by quantum computers as DH (only
    subexponential attacks known);
  \item Construction possible on \emph{graphs of ordinary curves}.
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{A ZK proof of knowledge\footcite{defeo+jao+plut12}}

...
\usecolortheme[RGB={113,43,10}]{structure}
\definecolor{title}{RGB}{120,230,100}
\setbeamercolor*{title}{fg=title}
\setbeamercolor*{titlegraphic}{fg=yellow!90!black}
...
\title[Isogeny-based cryptography]{A review of isogeny based cryptography}
\author{Luca De Feo}
\date[AfricaCrypt 2017]{May 25, 2017, AfricaCrypt, Dakar}
...
\frame[plain]{
  \begin{tikzpicture}[remember picture,overlay]
    \node[at=(current page.center), opacity=0.7] {
      \includegraphics[width=1.2\paperwidth]{polly.jpg}
    };
  \end{tikzpicture}
  \vspace{-3.5cm}
...
\begin{frame}
  ...
  \begin{block}{The explicit isogeny problem}
    ...
    \item[Instances]
      \begin{itemize}
      \item \emph{Input = kernel generator }
        {\usebeamertemplate{itemize subitem}}
        Velu's formulas; \hfill $\tildO(n)$
      \item \emph{Input = $E$ and $E/H$ }
        \begin{itemize}
        \item Elkies' algorithm\footcite{elkies98} (and variants); \hfill $\tildO(n)$
        \item Couveignes' algorithm\footcite{couveignes96} (and variants). \hfill $\tildO(n^2)$
        \end{itemize}
      \end{itemize}
...
\begin{frame}
  ...
  \begin{block}{The isogeny evaluation problem}
    ...
    \item[Examples]
      ...
      \item \emph{Input =} kernel generator; \hfill $O(??)$
      \item \emph{Input =} $\phi(a^{-1}P)$; \hfill $O(1)$
      \end{itemize}
    \end{description}
  \end{block}
...
\begin{frame}
  \frametitle{Key exchange in graphs of ordinary isogenies\footcite{Couv,R&S}}
  
  \emph{Parameters:}
  \begin{itemize}
  \item \alert{$E/\F_p$ ordinary elliptic curve} with \emph{Frobenius
      endomorphism} $\pi$,
...
\begin{frame}
  ...
  \item[Quantum\footcite{childs+jao+soukharev10}:] Shor + isogeny
...
\begin{frame}
  ...
  \textbf{Open problem:} exploit the \emph{additional information}
  transmitted by the protocol to improve attacks (classical or
  quantum).
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Other security models}
    ...
    \item[Side channel] Constant-time implementation
      available.\footcite{costello2016sidh}
    \item[] Attack on partially leaked
      keys.\footnotemark[1]\footnote{Two more papers at PQCrypto 2017}
    \end{description}
  \end{block}

  \textbf{Open problem:} Create a protocol secure against \emph{active
    adversaries}.
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Interactive protocols}
    \begin{itemize}
    \item Signatures (using Fiat-Shamir)\footcite{galbraithsecurity},
  ...
  \textbf{Open problem:} Classical signatures, \dots
\end{frame}

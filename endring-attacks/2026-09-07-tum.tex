...
\title[The supersingular endomorphism ring problem]{How to solve the supersingular endomorphism ring problem}
\author{Luca De Feo}
\date[September 7, 2026, MathPQC, Raitenhaslach]{September 7, 2026, Mathematics of Post-Quantum Cryptography, Raitenhaslach}
...
\begin{frame}{Collision search: Galbraith '99}
  \large\centering
  \begin{tikzpicture}
    \coordinate (s) at (0,0);
    \coordinate (e) at (s);
...
\begin{frame}[plain]
  \begin{tikzpicture}[overlay,remember picture]
    \draw (current page.center) node {\includegraphics[width=\paperwidth]{mue-bur.jpg}};
  \end{tikzpicture}
\end{frame}

%%

\begin{frame}{Collision search: Delfs-Galbraith '16}
  \large
  \begin{tikzpicture}[overlay,remember picture]
...
\begin{frame}{Reflections (Chenu-Smith '21)}
  \large\centering
  \begin{tikzpicture}
...
\begin{frame}{Squared minima of random $\Hom(E,E')$, log scale}
  ...
  \begin{tikzpicture}
    \draw
    (0,0) node{\includegraphics[width=0.47\textwidth]{hom_d1,4.png}}
    +(0,-3) node{1st and 4th minimum}
    (8,0) node{\includegraphics[width=0.47\textwidth]{hom_d2,3.png}}
...
\begin{frame}{How hard is it to find a short isogeny? \small(Folklore)}
  \large\centering
  \begin{tikzpicture}
    \node (E) at (0,0) {$E$};
    \node (E1) at (10,0) {$E'$};
...
\begin{frame}{Wesolowski's attack '26}
  \large\centering
  \begin{tikzpicture}
    \node (E) at (0,0) {$E$};
    \node (E1) at (10,0) {$E^{(p)}$};
  ...
  \item $⇒$ $O(n^2)$ isogenies of degree $≤ n$ from $E$
  \item<2-> $⇒$ If we're lucky, we can meet-in-the-middle at cost
    $O(p^{1/3})$ time \uncover<4->{\textbf{and memory!}}
  \end{itemize}
\end{frame}

%%

\begin{frame}{How to cost Wesolowski's attack?}
  \large
  \begin{description}
    \setlength{\itemsep}{2em}
  \item[Invariant:]
    $T^2M = \tilde{O}(p)$ via\\[1em]
    \begin{itemize}
    \item van Oorschot--Wiener
    \item ``going under'' (Chenu-Smith)
    \end{itemize}
  \item[At constant memory:] $T = \tilde{O}(\sqrt{p})$
  \item[With memory bound $w$:] $T = \tilde{O}(\sqrt{p/w})$
  \end{description}
\end{frame}

%%

\begin{frame}{SQIsign Round 3}
  \centering
  \begin{tabular}{l >{\bf}c c | c | >{\bf}c | c c}
  \toprule
    & {\sf old $p$} & new $p$ & memory & $\sqrt{p/w}$ & {PK} & {Sig}  \\
    & \multicolumn{2}{c|}{\it bits} & {\it bound} & {\sf\it bits} & \multicolumn{2}{c}{\it bytes}\\ 
  \midrule
  {NIST\nobreakdash-I}   & 250.3 & 325.6 &  $2^{80}$ & 245.6 &  83 & 200 \\ 
  {NIST\nobreakdash-III} & 382.0 & 504.8 & $2^{120}$ & 384.8 & 129 & 306 \\ 
  {NIST\nobreakdash-V}   & 504.8 & 668.1 & $2^{160}$ & 508.1 & 169 & 406 \\
  \bottomrule
  \end{tabular}
  ...
  \begin{tabular}{l | c c c}
    ...
    & \multicolumn{3}{c}{\it Mcycles}\\
    \midrule
    {NIST\nobreakdash-I}   & 32.2 & 93.9 & 12.1 \\
    {NIST\nobreakdash-III} & 102.1 & 320.3 & 31.5 \\
    {NIST\nobreakdash-V}   & 214.5 & 674.7 & 77.6 \\
    \bottomrule
    \end{tabular}
\end{frame}
